...
checksums="$(curl -fsSL "$release_url/checksums.txt")" \
...
want = {
    "recall_darwin_arm64.tar.gz": None,
    "recall_darwin_amd64.tar.gz": None,
    "recall_linux_arm64.tar.gz": None,
    "recall_linux_amd64.tar.gz": None,
}
...
# Each sha256 belongs to the archive named in the url line above it, so they
# are rewritten as pairs. Replacing them positionally would silently hand a
# platform another platform's hash, and brew would report a corrupt download.
for name, digest in want.items():
    pattern = re.compile(
        r'(url "[^"]*/' + re.escape(name) + r'"\n\s*sha256 ")[a-f0-9]*(")'
    )
    s, n = pattern.subn(lambda m: m.group(1) + digest + m.group(2), s)
    if n != 1:
        sys.exit(f"expected exactly one url/sha256 pair for {name}, found {n}")

open(p, "w").write(s)
print(f"    {len(want)} checksums written")
PY
[ $? -eq 0 ] || die "could not update Formula/recall.rb"
...
if confirm "push the formula to $TAP"; then
  ...
  git clone --depth 1 "https://github.com/$TAP.git" "$tapdir/tap" --quiet \
    || die "could not clone $TAP"

  {
    printf '# Generated by scripts/release.sh in %s. DO NOT EDIT.\n' "$REPO"
    printf '# Change Formula/recall.rb there and cut a release.\n'
    cat Formula/recall.rb
  } >"$tapdir/tap/Formula/recall.rb"

  if git -C "$tapdir/tap" diff --quiet -- Formula/recall.rb; then
    warn "the tap already has this exact formula — nothing to push"
  else
    git -C "$tapdir/tap" add Formula/recall.rb
    git -C "$tapdir/tap" commit -q -m "recall $VERSION" || die "commit failed"
    git -C "$tapdir/tap" push -q || die "could not push to $TAP"
    ok "pushed — brew install pimlabs/tap/recall now resolves $VERSION"
  fi
else
