...
//! `claude` CLI.
//!
//! It lives in `recall-worker` because that is where merging is going: a
//! worker drains the server's merge queue, so the `claude` login can leave
//! the process that faces the internet. Until every deployment has a
//! worker, `recall-server` still merges inline when none is enrolled, and
//! uses this module for it, built without the worker's HTTP client (the
//! `client` feature), so the server gains no client code.
//!
