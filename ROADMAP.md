...
- [ ] **Delete/tombstone support.** Same pass found `recall-push` silently no-ops on a local delete (`[[ -f "$FILE_PATH" ]] || exit 0`) — the server never learns a file was removed, so it comes back on the next pull. Needs a real design (a delete endpoint, and deciding whether "deleted" is a tombstone row or an actual row removal) before building it — not done yet.
