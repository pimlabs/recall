...
- `/v1/devices` and `/v1/enroll-keys` (0.4.1) — enrolling a machine as a device with its own key, and approving, listing and revoking devices and the enrolment keys cloud sessions enrol with. They change state about devices only; none of them reads or writes memory.
